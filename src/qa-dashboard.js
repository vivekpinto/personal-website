...
const SUMMARY_URL = 'https://vivekpinto.github.io/personal-website/test-report/test-summary.json';
...
function renderTestCatalog() {
  ...
  const automatedTests = testCatalog.reduce(
    (total, category) =>
      total +
      category.tests.filter((test) => test.automated).length,
    0
  );
...
/*
 * Render the automation status.
 */
function renderAutomationStatus(summary) {
  const container =
    document.querySelector('#automation-status');
...
async function loadAutomationStatus() {
  ...
  try {
    const summary = await fetchTestSummary();

    /*
     * Remember the currently displayed report.
     * We will use this when a new test run starts.
     */
    previousGeneratedAt =
...
async function runAutomationTests() {
  ...
  /*
   * Get the timestamp BEFORE starting the new run.
   */
  let startingGeneratedAt =
    previousGeneratedAt;

  /*
   * Make sure we have the latest existing timestamp.
   */
  try {
...
/*
 * Poll only after the user starts a test run.
 */
function startAutomationStatusPolling(
  ...
  /*
   * Stop an existing polling process.
   */
  ...
  const poll = async () => {
    ...
    try {
      ...
      if (
        currentGeneratedAt &&
        currentGeneratedAt !== startingGeneratedAt
      ) {
        ...
        /*
         * Important:
         * Stop polling because the new result
         * has been received.
         */
        stopAutomationStatusPolling();
